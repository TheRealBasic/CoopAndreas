...
| Home Invasion | `scm/scripts/RYDER.txt` | `in progress` | Owner: Mission Sync Pod (Ryder stream). Dependency opcodes: `create_pickup`, `task_go_straight_to_coord`, `set_timers`. Active blockers: `HIN-BLK-01` stealth pickup replication harness pending, `HIN-BLK-02` timer authority assertion wiring pending, `HIN-BLK-03` straight-to-coord task mirror pending. | Checklist-driven gate tracking: [x] start/cutscene notes seeded, [x] objective notes seeded, [x] fail notes seeded, [x] pass notes seeded, [x] reconnect notes seeded, [x] late-join notes seeded; evidence scaffold in `docs/qa/storyline-wave-mission-evidence.md` (session placeholder `NET-W2-TBD-HIN`). |
| Catalyst | `scm/scripts/RYDER.txt` | `in progress` | Owner: Mission Sync Pod (Ryder stream). Dependency opcodes: `create_train`, `task_jump_from_car`, `set_mission_audio_position`. Active blockers: `CAT-BLK-01` train entity authority hook pending, `CAT-BLK-02` jump task transition replay guard pending, `CAT-BLK-03` mission-audio position replication pending. | Checklist-driven gate tracking: [x] start/cutscene notes seeded, [x] objective notes seeded, [x] fail notes seeded, [x] pass notes seeded, [x] reconnect notes seeded, [x] late-join notes seeded; evidence scaffold in `docs/qa/storyline-wave-mission-evidence.md` (session placeholder `NET-W2-TBD-CAT`). |
| Robbing Uncle Sam | `scm/scripts/RYDER.txt` | `in progress` | Owner: Mission Sync Pod (Ryder stream). Dependency opcodes: `create_object`, `attach_object_to_car`, `set_char_obj_kill_char_any_means`. Active blockers: `RUS-BLK-01` crate object lifecycle replication pending, `RUS-BLK-02` attach-to-vehicle state fan-out pending, `RUS-BLK-03` kill-objective once-only adjudication pending. | Checklist-driven gate tracking: [x] start/cutscene notes seeded, [x] objective notes seeded, [x] fail notes seeded, [x] pass notes seeded, [x] reconnect notes seeded, [x] late-join notes seeded; evidence scaffold in `docs/qa/storyline-wave-mission-evidence.md` (session placeholder `NET-W2-TBD-RUS`). |
