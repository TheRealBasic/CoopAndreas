...
| W2 | `RYDER` | Mission Sync Pod (Ryder stream) | `in progress` | `clear` (0 missing on 2026-03-30) | Same checkpoints `C1..C6`; `C3` satisfied on 2026-03-31 (start/objective/fail/pass/reconnect/late-join gates captured for `Home Invasion`, `Catalyst`, `Robbing Uncle Sam` with onboarding template reference) — evidence: `docs/qa/storyline-wave-mission-evidence.md` (W2 table). |
...
#### W2 dependency command confirmation (RYDER)

- `Home Invasion`: `create_pickup`, `task_go_straight_to_coord`, `set_timers` (confirmed).
- `Catalyst`: `create_train`, `task_jump_from_car`, `set_mission_audio_position` (confirmed).
- `Robbing Uncle Sam`: `create_object`, `attach_object_to_car`, `set_char_obj_kill_char_any_means` (confirmed).
...
| Home Invasion | `RYDER` | Mission Sync Pod (Ryder stream) | 2026-04-03 | `in progress -> done` |
| Catalyst | `RYDER` | Mission Sync Pod (Ryder stream) | 2026-04-03 | `in progress -> done` |
| Robbing Uncle Sam | `RYDER` | Mission Sync Pod (Ryder stream) | 2026-04-04 | `in progress -> done` |
...
#### W3 kickoff isolation queue (SMOKE)

Previously `not started` W3 missions are isolated in a focused kickoff lane so implementation + first QA pass can be tracked explicitly before broader W3 expansion:

| Mission | Current status | Owner | Priority command/opcode parity focus | Status transition guard |
| --- | --- | --- | --- | --- |
| OG Loc | `in progress` | Mission Sync Pod (Smoke stream) | `task_follow_nav_mesh_to_coord`, `task_bike_flee_point`, `set_objective` | Entered `in progress` after implementation kickoff + first QA pass scaffold; move to `done` only after `docs/qa/storyline-mission-signoff-checklist.md` is complete. |
| Wrong Side Of The Tracks | `in progress` | Mission Sync Pod (Smoke stream) | `create_train`, `task_drive_by`, `task_car_mission` + reconnect hydration proof | Entered `in progress` after implementation kickoff + first QA pass scaffold; move to `done` only after `docs/qa/storyline-mission-signoff-checklist.md` is complete. |
...
| Drive-By | `scm/scripts/SWEET.txt` | `done` (2026-03-31; checklist: `docs/qa/storyline-mission-signoff-checklist.md`) | `create_car`, `task_drive_by`, `set_objective` | Wave W1 mission sync completed with all six gates `pass` in `docs/qa/storyline-wave-mission-evidence.md`; mini ticket: [`docs/qa/storyline-shared-command-mini-tickets.md#drive-by-scmscriptssweettxt`](docs/qa/storyline-shared-command-mini-tickets.md#drive-by-scmscriptssweettxt). Mission evidence artifacts: `artifacts/qa/storyline/drive-by/NET-W1-2026-03-31-C/`; reused fixes: `WB-FIX-003`, `WB-FIX-004`. |
...
| Burning Desire | `scm/scripts/CRASH.txt` | `in progress` | Owner: Mission Sync Pod (CRASH stream). Dependency opcodes: `create_fire`, `remove_char_from_car_maintain_position`, `set_char_obj_leave_car` (verified via `COMMAND_TASK_LEAVE_CAR` / `COMMAND_TASK_LEAVE_ANY_CAR` sync paths). Active blockers: `BD-BLK-01` fire lifecycle replay traces pending, `BD-BLK-02` forced vehicle-exit objective transition parity pending, `BD-BLK-03` burning-building fail/pass terminal once-only proof pending. | Checklist-driven gate tracking: [x] start/cutscene notes seeded, [x] objective notes seeded, [x] fail notes seeded, [x] pass notes seeded, [x] reconnect notes seeded, [x] late-join notes seeded; evidence scaffold in `docs/qa/storyline-wave-mission-evidence.md` (session placeholder `NET-WC-TBD-BD`). Move to `done` only after blockers `BD-BLK-01..03` are closed and `docs/qa/storyline-mission-signoff-checklist.md` is complete. |
| Gray Imports | `scm/scripts/CRASH.txt` | `in progress` | Owner: Mission Sync Pod (CRASH stream). Dependency opcodes: `create_actor`, `set_char_obj_kill_char_any_means`, `blow_up_car`. Active blockers: `GI-BLK-01` warehouse actor spawn authority trace pending, `GI-BLK-02` kill-objective + blast-transition monotonicity proof pending, `GI-BLK-03` vehicle detonation reconnect/late-join hydration parity pending. | Checklist-driven gate tracking: [x] start/cutscene notes seeded, [x] objective notes seeded, [x] fail notes seeded, [x] pass notes seeded, [x] reconnect notes seeded, [x] late-join notes seeded; evidence scaffold in `docs/qa/storyline-wave-mission-evidence.md` (session placeholder `NET-WC-TBD-GI`). Move to `done` only after blockers `GI-BLK-01..03` are closed and `docs/qa/storyline-mission-signoff-checklist.md` is complete. |
| Home Invasion | `scm/scripts/RYDER.txt` | `in progress` | Owner: Mission Sync Pod (Ryder stream). Dependency opcodes: `create_pickup`, `task_go_straight_to_coord`, `set_timers`. Active blockers: `HIN-BLK-01` stealth pickup replication harness pending, `HIN-BLK-02` timer authority assertion wiring pending, `HIN-BLK-03` straight-to-coord task mirror pending. Active implementation started with initial QA trace set `NET-W2-2026-03-31-A`; shared guards explicitly reused: unresolved-entity deferral/rebind retry (`WB-FIX-001`) + objective monotonic replay latch (`WB-FIX-002`) + terminal once-only adjudication (`WB-FIX-003`) + reconnect hydration epoch filter (`WB-FIX-004`). | Checklist-driven gate tracking: [x] start/cutscene notes seeded, [x] objective notes seeded, [x] fail notes seeded, [x] pass notes seeded, [x] reconnect notes seeded, [x] late-join notes seeded; evidence scaffold in `docs/qa/storyline-wave-mission-evidence.md` (session placeholder `NET-W2-TBD-HIN`). |
| Catalyst | `scm/scripts/RYDER.txt` | `in progress` | Owner: Mission Sync Pod (Ryder stream). Dependency opcodes: `create_train`, `task_jump_from_car`, `set_mission_audio_position`. Active blockers: `CAT-BLK-01` train entity authority hook pending, `CAT-BLK-02` jump task transition replay guard pending, `CAT-BLK-03` mission-audio position replication pending. Active implementation started with initial QA trace set `NET-W2-2026-03-31-B`; shared guards explicitly reused: unresolved-entity deferral/rebind retry (`WB-FIX-001`) + objective monotonic replay latch (`WB-FIX-002`) + terminal once-only adjudication (`WB-FIX-003`) + reconnect hydration epoch filter (`WB-FIX-004`). | Checklist-driven gate tracking: [x] start/cutscene notes seeded, [x] objective notes seeded, [x] fail notes seeded, [x] pass notes seeded, [x] reconnect notes seeded, [x] late-join notes seeded; evidence scaffold in `docs/qa/storyline-wave-mission-evidence.md` (session placeholder `NET-W2-TBD-CAT`). |

DoD promotion hold (W2 RYDER): keep all three missions at `in progress` until six-gate `pass` evidence and `docs/qa/storyline-mission-signoff-checklist.md` completion are recorded; then promote to `done` with explicit completion date and checklist citation.

| Robbing Uncle Sam | `scm/scripts/RYDER.txt` | `in progress` | Owner: Mission Sync Pod (Ryder stream). Dependency opcodes: `create_object`, `attach_object_to_car`, `set_char_obj_kill_char_any_means`. Active blockers: `RUS-BLK-01` crate object lifecycle replication pending, `RUS-BLK-02` attach-to-vehicle state fan-out pending, `RUS-BLK-03` kill-objective once-only adjudication pending. Active implementation started with initial QA trace set `NET-W2-2026-03-31-C`; shared guards explicitly reused: unresolved-entity deferral/rebind retry (`WB-FIX-001`) + objective monotonic replay latch (`WB-FIX-002`) + terminal once-only adjudication (`WB-FIX-003`) + reconnect hydration epoch filter (`WB-FIX-004`). | Checklist-driven gate tracking: [x] start/cutscene notes seeded, [x] objective notes seeded, [x] fail notes seeded, [x] pass notes seeded, [x] reconnect notes seeded, [x] late-join notes seeded; evidence scaffold in `docs/qa/storyline-wave-mission-evidence.md` (session placeholder `NET-W2-TBD-RUS`). |
| OG Loc | `scm/scripts/SMOKE.txt` | `in progress` | Owner: Mission Sync Pod (Smoke stream). Dependency opcodes: `task_follow_nav_mesh_to_coord`, `task_bike_flee_point`, `set_objective`. Active blockers: `OGL-BLK-01` nav-mesh path stage mirror pending, `OGL-BLK-02` bike flee-point task parity pending, `OGL-BLK-03` objective fan-out parity pending. | Checklist-driven gate tracking: [x] start/cutscene notes seeded, [x] objective notes seeded, [x] fail notes seeded, [x] pass notes seeded, [x] reconnect notes seeded, [x] late-join notes seeded; evidence scaffold in `docs/qa/storyline-wave-mission-evidence.md` (session placeholder `NET-W3-TBD-OGL`). Promote to `done` only after sign-off checklist completion in `docs/qa/storyline-mission-signoff-checklist.md`. |
...
| Wrong Side Of The Tracks | `scm/scripts/SMOKE.txt` | `in progress` | Owner: Mission Sync Pod (Smoke stream). Dependency opcodes: `create_train`, `task_drive_by`, `task_car_mission`. Active blockers: `WST-BLK-01` train authority spawn replication pending, `WST-BLK-02` drive-by task sequencing parity pending, `WST-BLK-03` car-mission reconnect hydration pending. | Checklist-driven gate tracking: [x] start/cutscene notes seeded, [x] objective notes seeded, [x] fail notes seeded, [x] pass notes seeded, [x] reconnect notes seeded, [x] late-join notes seeded; evidence scaffold in `docs/qa/storyline-wave-mission-evidence.md` (session placeholder `NET-W3-TBD-WST`). Promote to `done` only after sign-off checklist completion in `docs/qa/storyline-mission-signoff-checklist.md`. |
