...
| Mission | Status | Start/cutscene validation | Objective validation | Fail validation | Pass validation | Reconnect validation | Late-join validation | Notes / evidence refs |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Home Invasion | `in progress` | `in progress` — `HIN-01` start/cutscene handshake capture started in `NET-W2-TBD-HIN` (placeholder). | `in progress` — `HIN-02` stealth-objective progression trace checklist scaffolded against dependency opcodes. | `in progress` — `HIN-04` fail branch (`noise/alert` + `player_death`) checklist scaffolded. | `in progress` — `HIN-05` pass branch + reward idempotency checklist scaffolded. | `in progress` — `HIN-06` reconnect restore checklist scaffolded for burglary phase replay. | `in progress` — `HIN-07` late-join hydration checklist scaffolded for active objective phase. | Owner: Mission Sync Pod (Ryder stream). Session: `NET-W2-TBD-HIN`. Blocking command focus order: `create_pickup` -> `task_go_straight_to_coord` -> `set_timers`. Gate checklist scaffold: [ ] start/cutscene pass, [ ] objective pass, [ ] fail pass, [ ] pass pass, [ ] reconnect pass, [ ] late-join pass. Onboarding template: `docs/qa/storyline-mission-template.md` (C3). |
| Catalyst | `in progress` | `in progress` — `CAT-01` launch/cutscene sync capture started in `NET-W2-TBD-CAT` (placeholder). | `in progress` — `CAT-02` chase/jump objective checklist scaffolded against dependency opcodes. | `in progress` — `CAT-04` fail branch (`target_escape` + `player_death`) checklist scaffolded. | `in progress` — `CAT-05` pass branch + terminal once-only guard checklist scaffolded. | `in progress` — `CAT-06` reconnect restore checklist scaffolded for mid-chase phase replay. | `in progress` — `CAT-07` late-join hydration checklist scaffolded for active chase phase. | Owner: Mission Sync Pod (Ryder stream). Session: `NET-W2-TBD-CAT`. Blocking command focus order: `create_train` -> `task_jump_from_car` -> `set_mission_audio_position`. Gate checklist scaffold: [ ] start/cutscene pass, [ ] objective pass, [ ] fail pass, [ ] pass pass, [ ] reconnect pass, [ ] late-join pass. Onboarding template: `docs/qa/storyline-mission-template.md` (C3). |
| Robbing Uncle Sam | `in progress` | `in progress` — `RUS-01` start/cutscene sync capture started in `NET-W2-TBD-RUS` (placeholder). | `in progress` — `RUS-02` forklift/crate objective checklist scaffolded against dependency opcodes. | `in progress` — `RUS-04` fail branch (`crate_loss` + `player_death`) checklist scaffolded. | `in progress` — `RUS-05` pass branch + reward idempotency checklist scaffolded. | `in progress` — `RUS-06` reconnect restore checklist scaffolded for crate-load stage replay. | `in progress` — `RUS-07` late-join hydration checklist scaffolded for active crate phase. | Owner: Mission Sync Pod (Ryder stream). Session: `NET-W2-TBD-RUS`. Blocking command focus order: `create_object` -> `attach_object_to_car` -> `set_char_obj_kill_char_any_means`. Gate checklist scaffold: [ ] start/cutscene pass, [ ] objective pass, [ ] fail pass, [ ] pass pass, [ ] reconnect pass, [ ] late-join pass. Onboarding template: `docs/qa/storyline-mission-template.md` (C3). |
